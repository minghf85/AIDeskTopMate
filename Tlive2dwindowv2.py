...
class TransparentLive2dWindow(QOpenGLWidget):
    ...
    def _load_model(self, model_path="kurisu/kurisu.model.json"):
    ...
    def mouseMoveEvent(self, event: QMouseEvent) -> None:
        if self.dragging_window and self.drag_start_pos is not None:
...
class Live2dController:
    def __init__(self, window: TransparentLive2dWindow, model: live2d.LAppModel):
        self.window = window
        self.model = model
    def load_model(self, model_path: str):
        try:
            self.model.LoadModelJson(model_path)
            self.model.Resize(self.window.screen_width, self.window.screen_height)
            print(f"Model loaded: {model_path}")
        except Exception as e:
            print(f"Failed to load model: {e}")
